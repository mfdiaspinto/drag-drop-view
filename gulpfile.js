...
const appFolder = path.join(rootFolder, 'dist/app');
...
gulp.task('copy:source', function () {
    return gulp.src([`${srcFolder}/**/*`, `!${srcFolder}/node_modules/**/*`, `!${srcFolder}/app/**/*`])
        .pipe(chmod(666))
...
gulp.task('inline-resources', async function () {
    return await inlineResources(tmpFolder);
});
...
/** 
 * 11. Copy scss from source to dist
 */
...
gulp.task('clean:app', function () {
    return deleteFolders([appFolder]);
});
    ...
    runSequence(
        ...
        function (err) {
            if (err) {
                ...
                console.log('\n\nERROR in \'compile\' gulp task the DIST folder was deleted!');
            } else {
